...
class Config:
    ...
    def read_pretrain_embedding(self) -> Tuple[Union[Dict[str, np.array], None], int]:
        ...
        embedding = dict()            
#         model = Word2Vec.load("/home/jatin-pg/BTP/materials-word-embeddings-master/bin/word2vec_embeddings-SNAPSHOT.model")
        model = Word2Vec.load("/home/jatin-pg/BTP/pytorch_lstmcrf-master/pretrained_embeddings")
        wordvec = model.wv
        for word in tqdm(wordvec.vocab):
#             line = line.strip("\n")
#             if len(line) == 0:
#                 continue
#             tokens = line.split()
...
#                 assert (embedding_dim + 1 == len(tokens))
            ...
            embedd = np.empty([1, self.embedding_dim])
            embedd[:] = wordvec[word]
            first_col = word
