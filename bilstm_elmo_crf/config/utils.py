...
def load_elmo_vec(file: str, insts: List[Instance]):
    ...
    for vec, inst in zip(all_vecs, insts):
        ...
        # print(vec.shape)
        # print(len(inst.input.words))
        if(vec.shape[0] != len(inst.input.words)):
            print(vec.shape, len(inst.input.words))
            # temp = np.random.rand(len(inst.input.words) - vec.shape[0],vec.shape[1])
            # vec = np.concatenate((vec,temp), axis=0)
...
def write_results(filename: str, insts):
    ...
    for inst in insts:
        ...
        for i in range(len(inst.input)):
            f.write("{} {} {}\n".format(words[i], prediction[i].replace('S-','B-').replace('E-','I-'),output[i].replace('S','B').replace('E-','I-')))
            y_pred.append(prediction[i].replace('S-','B-').replace('E-','I-'))
            y_true.append(output[i].replace('S-','B-').replace('E-','I-'))
        f.write("\n")
    f.close()